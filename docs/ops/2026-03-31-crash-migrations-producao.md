...
1. Container `mirante_backend_prod` em restart loop contínuo
2. Log do container:
   ```
...
- name: Build binary
  ...
  run: |
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -mod=vendor \
      -ldflags "-X github.com/nilbyte/mirante/backend/internal/version.Version=${{ needs.bump-versions.outputs.BACKEND_VERSION }}" \
      -o main ./cmd/api
...
O `docker-compose.backend.yml` do servidor tinha apenas o serviço `mirante-backend`. Não havia nenhum serviço de migration no compose. O banco subia vazio e o backend tentava usar tabelas que não existiam.

### Fix aplicado

Adicionado serviço `mirante-migrate` ao `docker-compose.backend.yml`:
...
```
mirante-db (healthy) → mirante-migrate (completed) → mirante-backend (up)
```
...
```
dial tcp: lookup mirante_db_prod on 127.0.0.11:53: server misbehaving
```

### Por que acontecia

O `manage-mirante.sh` subia o banco separadamente com `docker-compose.db.yml` e depois subia backend+webapp com `docker-compose.backend.yml -f docker-compose.webapp.yml --remove-orphans`. O flag `--remove-orphans` **removia o container `mirante_db_prod`** por ele não estar nos compose files de backend/webapp. Quando o migrate tentava conectar, o banco já tinha sido destruído.
...
**2. Script `manage-mirante.sh` — subir tudo junto:**

```sh
start|up)
    docker network create mirante-internal 2>/dev/null || true
...
```sh
sh scripts/manage-mirante.sh seed
...
O case `seed` no `manage-mirante.sh` rodava o container sem argumento:
...
```
[+] up 4/4
 ✔ Container mirante_db_prod      Healthy    6.4s
 ✔ Container mirante_migrate_prod Exited     7.0s   ← ✅ Migrations applied (version 4)
 ✔ Container mirante_webapp_prod  Created
 ✔ Container mirante_backend_prod Created
```
